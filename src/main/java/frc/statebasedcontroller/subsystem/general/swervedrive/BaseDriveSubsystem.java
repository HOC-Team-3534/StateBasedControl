...
/**
 * An extension of {@link BaseSubsystem}, the base drive subsystem is a subsystem for swerve drive robots.
 * It accomodates for the additional functionality required by the control system of the swerve drive modules as a whole.
 * It can track the pose of the robot, follow a path autonomously, and control the robot manually.
 */
public abstract class BaseDriveSubsystem<SsS extends ISubsystemState> extends BaseSubsystem<SsS> {
    ...
    /**
     * 
     * @param dt the drivetrain model that encompasses the modules and allows for tracking and control
     * @param kinematics the physical locations of the swerve modules relative to the center of the robot
     ...
     * 
     * additionally, the drive subsystem keeps track of telemetry, send the desired states of the modules to the modules
     * and updates the simulation if in simulation mode
     */
    public void process() {
        dt.updateTelemetry();
        super.process();
    ...
    /**
     * Sends the desired module states to the swerve modules using the drivetrain model.
     * Simultaneously updates pose estimation
     */
    ...
    /**
     * 
     * @return the path currently being followed by the drive subsystem during autonomous
     */
     ...
     * 
     * @param ppf the desired path follower to follow during phase of autonomous sequence
     * @param setInitialPosition should the pose of the robot be reset to the initial pose of the path
     */
    ...
    /**
     * 
     * @return get velocity vector of the robot with respect to the robot in meters per second (m/s)
     */
    ...
    /**
     * 
     * @return get velocity vecotr of the robot with respect to the target in meters per second (m/s)
     */
    ...
    /**
     * Set the rotation the robot should turn to in order to aim the front at the target
     * 
     * @param angleErrorFunction typically using vision, the function that returns the offset to aim at target
     */
    ...
    /**
     * reset the gyroscope rotation to zero to indicate the front is pointing straight ahead
     */
     ...
     * 
     * @param chassisSpeeds the speed in each resprect that the robot that is currently desired
     */
     ...
     * 
     * @param x the proportion (-1 to 1) in the backward and forward direction to drive the robot
     * @param y the proportion (-1 to 1) in the left and right direction to drive the robot
     * @param rot the proportion (-1 to 1) in the counterclockwise and clockwise direction to drive the robot
     */
     ...
     * 
     * @param x the proportion (-1 to 1) in the backward and forward direction to drive the robot cut to creep proportion
     * @param y the proportion (-1 to 1) in the left and right direction to drive the robot cut to creep proportion
     * @param rot the proportion (-1 to 1) in the counterclockwise and clockwise direction to drive the robot cut to creep proportion
     */
    ...
    /**
     * Set the desired module states during aiming based on proportional inputs, typically
     * from a controller or similar device, but with creep proportions
     * 
     * @param x the proportion (-1 to 1) in the backward and forward direction to drive the robot cut to creep proportion
     * @param y the proportion (-1 to 1) in the left and right direction to drive the robot cut to creep proportion
    ...
    /**
     * Based on the current state of the path follower and the current pose,
     * the holonomic controller of the drivetrain model will calculate the 
