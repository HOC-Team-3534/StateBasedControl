...
/**
 * This base autononomous sequences class adds onto what {@link BaseSequence}
 * has to control the robot during autonous, which requires path following
 * abilities.
 */
public abstract class BaseAutonSequence<SeqP extends ISequencePhase> extends BaseSequence<SeqP> {
    ...
    /**
     * @param neutralPhase   the phase, usually enum labeled NEUTRAL, that has no
     *                       required subsystems
     * @param startPhase     the first phase to go to when the sequence begins
     *                       running/processing
     * @param driveSubsystem the drive subsytem of the robot
     */
    public BaseAutonSequence(SeqP neutralPhase, SeqP startPhase,
     ...
     * phase if successful. In addition to basic functionality, this also makes sure
     * to set the path for following because this sequence is autonomous
     * 
    ...
    /**
     * On the first run through of the phase, the path follower is set and reset to
     * the start, and then given to the drivesubsystem to use for autonomous path
     * following
     * 
     * @param setInitialPositionAndHeading whether to set the pose of the robot to
     *                                     the initial pose of the path, done at the
     *                                     first path of the auton sequence
     */
    void setPathPlannerFollowerAtStartOfState(boolean setInitialPositionAndHeading) {
        if (getPhaseFirstRunThrough()) {
            ...
            if (getPathPlannerFollower() != null) {
    ...
    /**
     * @param paths the paths for the auton sequence to follow, corresponding with
     *              the path indeces in the sequence phases
     */
