...
/**
 * This base sequence class should be extended by all teleop sequences (auton
 * sequences should extend {@link BaseAutonSequence} instead). Sequences are a
 * combination of {@link SequencePhase phases}. The phase can be switched using
 * custom logic in {@link #process()}. Sequences keep track of time for
 * convenience.
 */
public abstract class BaseSequence<SeqP extends ISequencePhase> implements ISequence<SeqP> {
    ...
    /**
     * @param neutralPhase the phase, usually enum labeled "NETURAL", that has no
     *                     required subsystems
     * @param startPhase   the first phase to go to when the sequence begins
     *                     running/processing
     */
    ...
    /**
     * When the sequence is currently in its neutral phase, call this method to
     * begin the sequence at the start phase, which will require the subsystems to
     * go to the associated state
     * 
     * @param subsystems usually empty. subsystems that must abort their current
     *                   sequences, using {@link BaseSubsystem#forceRelease()
     *                   forceRelease}
     */
    ...
    /**
     * Must be called at the end of {@link #process()} in each sequence, otherwise
     * the phase will not change and the sequence will get stuck. If it is not
     * placed at the very end, the functionality will not work correctly for logic
     * that requires the first run through flag
     * 
    ...
    /**
     * This only returns the phase enum. For the phase information, call
     * {@link ISequencePhase#getPhase()} on top of this getPhase() method to get the
     * {@link SequencePhase} instance associated with the {@link ISequencePhase}
     * enum.
     * 
    ...
    /**
     * Requires all the the subsubsystems associated with the phase and sets the
     * phase if successful
     * 
    ...
    /**
     * @return the next phase of the sequence, which is used to direct the flow of
     *         phases within the sequence
     */
    SeqP getNextPhase() {
        return nextPhase;
    }
    ...
    /**
     * @param phase the phase to switch the sequence to instead of the current phase
     *              to control flow of the sequence
     */
    ...
    /**
     * @return the phase of the sequence where no subsystems are required, set at
     *         initialization
     */
    ...
    /**
     * @return the phase of the sequence to begin the flow of phases to control the
     *         states of the subsystems to control the robot, set at initialization
     */
    SeqP getStartPhase() {
        return startPhase;
    }
    ...
    /**
     * @return the time in milliseconds since the start of the current phase of the
     *         sequence
     */
    ...
    /**
     * @return is it the first time the sequence has been ran since the phase has
     *         change to the current phase
     */
